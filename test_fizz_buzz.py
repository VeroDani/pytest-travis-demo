...
def test_fb_callable_with_1_argument():
    fizzbuzz(1)
...
def test_fb_threefive_is_buzz(num):
    assert fizzbuzz(num) == 'fizzbuzz'

@pytest.mark.parametrize('num', ['', 1.5, [], 4+3j])
def test_fb_raises_typeerro_on_nonint(num):
    with pytest.raises(TypeError):
        fizzbuzz(num)

@pytest.mark.parametrize('num', [0, -1, -30])
def test_fb_raises_typeerro_on_nonpositive(num):
